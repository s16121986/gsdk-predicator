...
abstract class AbstractPredicate {
	...
	public function expression($value, $comparison = null): ?string {
		...
		return match (true) {
			$comparison instanceof Comparison\Grater => $comparison->expression(
				$this->getIdentifier(),
				$this->getLowerBound($value) ?? $this->formatValue($this->getValue($value))
			),
			$comparison instanceof Comparison\Less => $comparison->expression(
				$this->getIdentifier(),
				$this->getUpperBound($value) ?? $this->formatValue($this->getValue($value))
			),
			$comparison instanceof Comparison\Interval => $comparison->expression(
				$this->getIdentifier(),
				$this->getLowerBound($value),
				$this->getUpperBound($value)
			),
			default => $comparison->expression(
				$this->getIdentifier(),
